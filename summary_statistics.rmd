...
load("cluster_app/functional_groups.rda")
...
ggplot(data[grepl(g_group,v_gene)], aes(v_allele, fill = v_allele)) + geom_bar() + facet_grid(.~project) + labs(x = "allele", y = "# Individuals", fill = "") + 
  bbc_style()  + theme(legend.position = "none") + scale_fill_manual(values = pal %>% usecol(n = 10))
```
...
```{r}
ggplot(data[grepl(g_group,v_gene)], aes(v_allele, count, color = project)) + geom_boxplot() + geom_jitter() + labs(x = "allele", y = "# Sequences", color = "") + 
  bbc_style() + scale_color_manual(values = pal %>% usecol(n = 10))
```
...
tableData <- data_cutoff(data, func_groups, g, 2, or_allele) %>% group_by(zygousity_state, v_alleles_abc, v_allele) %>% dplyr::summarise(mean_freq = mean(freq)) %>% group_by(zygousity_state, v_alleles_abc) %>% dplyr::summarise(fractions = paste0(round(mean_freq, 3), collapse = ";"))
